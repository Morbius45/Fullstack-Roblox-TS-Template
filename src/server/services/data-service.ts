...
const playerCollection = createCollection<PlayerData>("PlayerData", {
	...
	validate: (data): data is PlayerData => {
		return data !== undefined && typeIs((data as PlayerData).joinedAt, "number");
	},
});
...
export class DataService {
	...
	private async onPlayerAdded(player: Player) {
		try {
			const document = await playerCollection.load(`Player_${player.UserId}`, [player.UserId]);
	...
	public updateData(player: Player, updater: (data: PlayerData) => PlayerData): boolean {
		const document = this.documents.get(player);
